...
- **HTTP server** (`proxy/`): routes are declared in `proxy/api.go` under `/api/v1` (raw data) and `/api/v2` (server-side-rendered responses where the user's role changes the output, e.g. `drug/handler_v2.go` builds role-gated sections; `cui/` holds the table/form UI schema types). Responses are cached in Redis via gin-cache with a zstd-compressing store (`proxy/cache.go`).
