...
func (s *Service) DumpRecommendationsFromVmedisToRedis(ctx context.Context) error {
	token, acquired, err := s.redisDB.AcquireRecommendationsLock(ctx)
	if err != nil {
		return fmt.Errorf("acquire procurement recommendations lock: %w", err)
	}

	if !acquired {
		log.Println("Procurement recommendations are already being generated by another process, skipping")
		return nil
	}

	defer func() {
		if err := s.redisDB.ReleaseRecommendationsLock(context.WithoutCancel(ctx), token); err != nil {
			log.Printf("Failed to release procurement recommendations lock: %s", err)
		}
	}()
...
func (s *Service) GetRecommendationStatus(ctx context.Context) (RecommendationStatusResponse, error) {
	locked, err := s.redisDB.IsRecommendationsLocked(ctx)
	if err != nil {
		return RecommendationStatusResponse{}, fmt.Errorf("get procurement recommendation status: %w", err)
	}

	status := RecommendationStatusIdle
	if locked {
		status = RecommendationStatusGenerating
	}

	return RecommendationStatusResponse{Status: status}, nil
}
